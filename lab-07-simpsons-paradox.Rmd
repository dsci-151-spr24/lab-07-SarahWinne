...
***This was an bservational study, because there are not controlled variables, but there are observations taken from a survey, and no treatments were given.***

2.  How many observations are in this dataset?
    What does each observation represent?

***There are `r nrow(Whickham)` observations in this dataset. Each observation represents one specific woman in the study.***
...
***There are `r ncol(Whickham)` variables in this dataset. Both 'outcome' and 'smoker' are character data with 2 leve factors, while age is numerical data.***
...
***This does not represent what I expected. It appears that those who smoked had a higher chance of being alive 20 years later.***
...
ggplot(data = Whickham, aes(y = outcome, fill = smoker)) +
  geom_bar(position = "fill") +
  facet_wrap(~age_cat) +
  ggtitle("Distribution of Outcome with Age")
```

***In the new plot, it appears that smokers have a higher probability of being dead after 20 years, in each age category, unlike what the visualization shows when the age category is not included.***
